...
#            ifdef RGB_MATRIX_SOLID_REACTIVE_GRADIENT_MODE
    hsv.v = scale16by8(g_rgb_timer, qadd8(rgb_matrix_config.speed, 8) >> 4) + dy / 4;
#            else
    if (rgb_matrix_config.hsv.s <= ((dy < 0 ? -1 * dy : dy) * 12)) {
        hsv.s = 0;
    } else {
        hsv.s = rgb_matrix_config.hsv.s - ((dy < 0 ? -1 * dy : dy) * 3);
    }
#            endif
